...
function pretrain_Q(
    ...
    println(optim.layers[1].layers[1].layers[1].layers[1])
    Optimisers.freeze!(optim.layers[1].layers[1].layers[1].layers[1])
    Optimisers.freeze!(optim.layers[1].layers[1].layers[2])
    Optimisers.freeze!(optim.layers[1].layers[2])
    ...
    for _ in ProgressBar(1:iter_num)
        ...
        # v_targ = (1 - gamma) * c + gamma * max.(c, Q_model(state_action_prime))
        argmin_Q = create_x_add_x_mul_u_Q_interval(Q_model, task.x_dim, task.u_dim, task.u_low, task.u_high)
