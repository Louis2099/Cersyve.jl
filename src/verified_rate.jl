...
function verify_rate(
    ...
    for i in ProgressBar(1:total)
        ...
        # Verify whether it's a safe set
...
function verify_safeset_rate(
    ...
    girds_bound = split_hyperrectangle(x_high, x_low, res, x_dim)
    Y = Complement(HPolyhedron([1 0; 0 -1], [0, 0]))
    safe_Y = Complement(HPolyhedron([1 0], [0]))
    verified = 0
    total = length(girds_bound)
    verify_t = 0
    sofar = 0
    ...
    for i in ProgressBar(1:total)
        ...
        X = Hyperrectangle(low=l, high=u)
        # Verify whether it's a safe set
        safe_problem = Problem(V_h_model, X, safe_Y)
        safe_t = @elapsed safe_res = verify(search_method, split_method, solver, safe_problem;
            collect_bound=true, start_values=con_start_values)
        @printf "Unsafety %s! Verification time: %.3fs\n" safe_res.status safe_t
        if safe_res.status != :holds
            safe = true
        else
            safe = false
            continue
...
function filter_verified_safe_set(file::String, Q_min::Any, task::Any)
    ...
    Q_values_l = Q_min(batch_l[1:task.x_dim, :])
    Q_values_u = Q_min(batch_u[1:task.x_dim, :])
    ...
    println("Number of safe bounds: $(n_safe)")
