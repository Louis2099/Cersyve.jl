function create_mlp(
    input_dim::Int64,
    output_dim::Int64,
    hidden_sizes::Vector{Int64},
)::Chain
    layers = []
    push!(layers, Dense(input_dim => hidden_sizes[1], relu))
    for i in 1:length(hidden_sizes) - 1
        push!(layers, Dense(hidden_sizes[i] => hidden_sizes[i + 1], relu))
    end
    push!(layers, Dense(hidden_sizes[end] => output_dim))
    return Chain(layers...)
end

function create_closed_loop_dynamics_model(
    f_model::Chain,
    pi_model::Any,
    data::Dict{String, Array{Float32}},
    x_low::Vector{Float32},
    x_high::Vector{Float32},
    u_dim::Int64,
)::Chain
    x_dim = length(x_low)
    return Chain(
        Parallel(+,
            Dense(Matrix{Float32}(I(x_dim))),
            Chain(
                Parallel(+,
                    Chain(
                        Dense(diagm(1 ./ data["x_std"]), -data["x_mean"] ./ data["x_std"]),
                        Dense(vcat(Matrix{Float32}(I(x_dim)), zeros(Float32, u_dim, x_dim))),
                    ),
                    Chain(
                        pi_model,
                        Dense(diagm(1 ./ data["u_std"]), -data["u_mean"] ./ data["u_std"]),
                        Dense(vcat(zeros(Float32, x_dim, u_dim), Matrix{Float32}(I(u_dim)))),
                    ),
                ),
                f_model,
                Dense(diagm(data["dx_std"]), data["dx_mean"]),
            ),
        ),
        # max(x, x_low) = relu(x - x_low) + x_low
        Dense(Matrix{Float32}(I(x_dim)), -x_low, relu),
        Dense(Matrix{Float32}(I(x_dim)), x_low),
        # min(x, x_high) = -max(-x, -x_high) = -relu(-x + x_high) + x_high
        Dense(-Matrix{Float32}(I(x_dim)), x_high, relu),
        Dense(-Matrix{Float32}(I(x_dim)), x_high),
    )
end


function create_non_linear_dynamics_model(
    f_model::Chain,
    data::Dict{String, Array{Float32}},
    x_low::Vector{Float32},
    x_high::Vector{Float32},
    u_dim::Int64,
    x_dim::Int64,
)
    """
    f_model: f([x u]) = dx
    for non-linear dynamics, create f([x u]) = x'
    return f([x u]) = x'
    """
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_dim = length(x_low)
    return Chain(
        Parallel(+,
            filter_x,
            Chain(
                Parallel(+,
                    Chain(
                        filter_x,
                        Dense(diagm(1 ./ data["x_std"]), -data["x_mean"] ./ data["x_std"]),
                        Dense(vcat(Matrix{Float32}(I(x_dim)), zeros(Float32, u_dim, x_dim))),
                    ),
                    Chain(
                        filter_u,
                        Dense(diagm(1 ./ data["u_std"]), -data["u_mean"] ./ data["u_std"]),
                        Dense(vcat(zeros(Float32, x_dim, u_dim), Matrix{Float32}(I(u_dim)))),
                    ),
                ),
                f_model,
                Dense(diagm(data["dx_std"]), data["dx_mean"]),
            ),
        ),
        # max(x, x_low) = relu(x - x_low) + x_low
        Dense(Matrix{Float32}(I(x_dim)), -x_low, relu),
        Dense(Matrix{Float32}(I(x_dim)), x_low),
        # min(x, x_high) = -max(-x, -x_high) = -relu(-x + x_high) + x_high
        Dense(-Matrix{Float32}(I(x_dim)), x_high, relu),
        Dense(-Matrix{Float32}(I(x_dim)), x_high),
    )
    return model
end

function create_value_constraint_model(V_model::Any, h_model::Any)::Chain
    return Chain(Parallel(+,
        Chain(V_model, Dense(Float32[1; 0;;])),
        Chain(h_model, Dense(Float32[0; 1;;])),
    ))
end

function create_value_next_value_model(V_model::Any, f_pi_model::Any)::Chain
    return Chain(Parallel(+,
        Chain(V_model, Dense(Float32[1; 0;;])),
        Chain(f_pi_model, V_model, Dense(Float32[0; 1;;])),
    ))
end


function create_filter_matrix(start_idx, end_idx, total_len)
    W = zeros(end_idx - start_idx + 1, total_len)
    for i in start_idx:end_idx
        # println(i - start_idx + 1)
        W[i - start_idx + 1, i] = 1.0
    end
    return W
end

function create_expand_matrix(start_idx, end_idx, expand_dim)
    input_dim = end_idx - start_idx + 1

    W = zeros(expand_dim, input_dim)
    for i in start_idx:end_idx
        # println(i - start_idx + 1)
        W[i, i - start_idx + 1] = 1.0
    end
    return W
end




# Create the model
# function create_parallel_affine_Q_interval(x_dim, u_dim, u_low, u_high)

    
#     # function filter_x(input)
#     #     return input[1:x_dim, :]
#     # end
    
#     # function filter_u(input)
#     #     if size(input, 1) == x_dim
#     #         return input
#     #     end
#     #     return input[x_dim+1:end, :]
#     # end
#     W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
#     b_x = zeros(x_dim)
#     filter_x = Dense(W_x, b_x)
#     W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
#     u_b = zeros(u_dim)
#     filter_u = Dense(W_u, u_b)

#     # start_idx, end_idx, expand_dim
#     b1_embedding_dim = 32
#     W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
#     b_expand_emb = zeros(b1_embedding_dim + u_dim)
#     expand_emb = Dense(W_expand_emb, b_expand_emb)

#     W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
#     b_expand_u = zeros(b1_embedding_dim + u_dim)
#     expand_u = Dense(W_expand_u, b_expand_u)


#     W_replace_u = zeros(u_dim, u_dim)
#     b_replace_low = u_low
#     b_replace_high = u_high
#     replace_u_low = Dense(W_replace_u, b_replace_low)
#     replace_u_high = Dense(W_replace_u, b_replace_high)

#     # Branch 1
#     b1 = Chain(
#         filter_x,
#         Dense(x_dim, 32, relu),
#         Dense(32, 32, relu),
#         Dense(32, 1)
#         # expand_emb
#     )
    
#     b_low = Chain(
#         filter_u,
#         replace_u_low,
#         Dense(u_dim, 1)
#         # expand_u
#     )
#     b_high = Chain(
#         filter_u,
#         replace_u_high,
#         Dense(u_dim, 1)
#     )

#     model = Chain(
#         Parallel(
#             +, 
#             b1,
#             b_low,
#             b_high
#         ),
#         # final_layer_interval  # Perform interval arithmetic here
#     )


#     return model
# end
function replace_weight_xu_luul(x_dim, u_dim)
    # given [x u] return [x u-min] [x u-max] in format luul
    W = Matrix{Float64}(I, x_dim + u_dim, x_dim + u_dim)
    for i in 1:u_dim
        W[x_dim + i, x_dim + i] = 0
    end
    new_W = [W; W; W; W]
    return new_W
end

function replace_bias_xu_luul(u_high, u_low, x_dim, u_dim)
    
    b = Float64.(
    [zeros(x_dim);
    u_low; 
    zeros(x_dim);
    u_high; 
    zeros(x_dim);
    u_high;
    zeros(x_dim);
    u_low;])
    return b
end

function replace_weight_xu_lu(x_dim, u_dim)
    # given [x u] return [x u-min] [x u-max] in format luul
    W = Matrix{Float64}(I, x_dim + u_dim, x_dim + u_dim)
    for i in 1:u_dim
        W[x_dim + i, x_dim + i] = 0
    end
    new_W = [W; W]
    return new_W
end

function replace_bias_xu_lu(u_high, u_low, x_dim, u_dim)
    b = Float64.(
    [zeros(x_dim);
    u_low; 
    zeros(x_dim);
    u_high
    ])
    return b
end

function weight_luul(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    # new_W = [
    #     W_plus  zeros(size(W))  W_minus  zeros(size(W));
    #     W_minus zeros(size(W))  W_plus   zeros(size(W));
    #     zeros(size(W)) W_plus  zeros(size(W))  W_minus;
    #     zeros(size(W)) W_minus zeros(size(W))  W_plus
    # ]
    new_W = [W_plus W_minus zeros(size(W)) zeros(size(W));
             zeros(size(W)) zeros(size(W)) W_plus W_minus;
             W_minus W_plus zeros(size(W)) zeros(size(W));
             zeros(size(W)) zeros(size(W)) W_minus W_plus]
    return new_W
end

function bias_luul(b::AbstractVector)
    new_b = [b; b; b; b]
    
    return new_b
end

function weight_lu(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    # new_W = [
    #     W_plus  zeros(size(W))  W_minus  zeros(size(W));
    #     W_minus zeros(size(W))  W_plus   zeros(size(W));
    #     zeros(size(W)) W_plus  zeros(size(W))  W_minus;
    #     zeros(size(W)) W_minus zeros(size(W))  W_plus
    # ]
    new_W = [W_plus W_minus;
            W_minus W_plus]
    return new_W
end

function weight_ul(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [zeros(size(W)) zeros(size(W)) W_plus W_minus;
            W_plus W_minus zeros(size(W)) zeros(size(W))]

    # println("new_W shape", size(new_W))
    return new_W
end

function bias_lu(b::AbstractVector)
    new_b = [b; b]
    
    return new_b
end


function weight_l(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [W_plus W_minus zeros(size(W)) zeros(size(W))]
    # println("new_W shape", size(new_W))
    return new_W
end

function weight_l_small(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [W_plus W_minus]
    # println("new_W shape", size(new_W))
    return new_W
end

function weight_u_small(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [W_minus W_plus]
    # println("new_W shape", size(new_W))
    return new_W
end

function weight_lu_big(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [W_plus W_minus zeros(size(W)) zeros(size(W));
            zeros(size(W)) zeros(size(W)) W_plus W_minus]
    # println("new_W shape", size(new_W))
    return new_W
end

function weight_lu_2_l(W::AbstractMatrix)
    # Split the weight matrix into positive and negative components
    W_plus = max.(W, 0)  # Element-wise maximum 
    W_minus = min.(W, 0) # Element-wise minimum
    
    # Construct the new weight matrix
    new_W = [W_plus W_minus]
    # println("new_W shape", size(new_W))
    return new_W
end


function weight_pos_neg(num_x::Int64)
    new_W = [Float64.(I(num_x)); -Float64.(I(num_x))]
    # println("new_W type", typeof(new_W))
    return new_W
end

function weight_neg(num_x::Int64)
    new_W = [Float64.(I(num_x)) zeros(size(I(num_x))); 
            zeros(size(I(num_x))) -Float64.(I(num_x))]
    # println("new_W type", typeof(new_W))
    return new_W
end

function weight_replicate_emb(num_emb::Int64)
    new_W = [Float64.(I(num_emb)); Float64.(I(num_emb))]
    return new_W
end

function bias_replicate_emb(bias::AbstractVector)
    new_b = [bias; bias]
    return new_b
end

function create_parallel_affine_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias

    x_2_x_luul = Dense(u_dim, 4*u_dim; bias = [u_low; u_high; u_high; u_low])
    x_2_x_luul.weight .= 0.0

    h1_w = affine_Q[1][2][2].weight
    h1_b = affine_Q[1][2][2].bias
    x_luul_2_h1_luul = Dense(weight_luul(h1_w), bias_luul(h1_b), relu)

    h2_w = affine_Q[1][2][3].weight
    h2_b = affine_Q[1][2][3].bias   
    h1_luul_2_h2_luul = Dense(weight_luul(h2_w), bias_luul(h2_b), relu)


    Q_w = affine_Q[1][2][4].weight
    Q_b = affine_Q[1][2][4].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_l = Dense(weight_l(Q_w), Q_b)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3)
        # expand_emb
    )

    b_low = Chain(
        filter_u,
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_l
    )
    model = Chain(
        Parallel(
            +, 
            b1,
            b_low
            
        )
    )


    return model
end

function create_mul_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias
    minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))

    x_2_x_luul = Dense(u_dim, 4*u_dim; bias = [u_low; u_high; u_high; u_low])
    x_2_x_luul.weight .= 0.0

    h1_w = affine_Q[1][2][2].weight
    h1_b = affine_Q[1][2][2].bias
    x_luul_2_h1_luul = Dense(weight_luul(h1_w), bias_luul(h1_b), relu)

    h2_w = affine_Q[1][2][3].weight
    h2_b = affine_Q[1][2][3].bias   
    h1_luul_2_h2_luul = Dense(weight_luul(h2_w), bias_luul(h2_b), relu)


    Q_w = affine_Q[1][2][4].weight
    Q_b = affine_Q[1][2][4].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_lu = Dense(weight_lu(Q_w), [Q_b;Q_b])


    sum_layer = Dense(ones(1, 2*output_emb_dim), zeros(1))
    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3),
        minmax_layer,
        neg_layer
    )

    b_low = Chain(
        filter_u,
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    model = Chain(
        Parallel(
            .*, 
            b1,
            b_low
            
        ),
        sum_layer

    )


    return model
end

function create_mul_affine_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias
    minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))

    x_2_x_luul = Dense(u_dim, 4*u_dim; bias = [u_low; u_high; u_high; u_low])
    x_2_x_luul.weight .= 0.0

    h1_w = affine_Q[1][2][2].weight
    h1_b = affine_Q[1][2][2].bias
    x_luul_2_h1_luul = Dense(weight_luul(h1_w), bias_luul(h1_b))

    h2_w = affine_Q[1][2][3].weight
    h2_b = affine_Q[1][2][3].bias   
    h1_luul_2_h2_luul = Dense(weight_luul(h2_w), bias_luul(h2_b))


    Q_w = affine_Q[1][2][4].weight
    Q_b = affine_Q[1][2][4].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_lu = Dense(weight_lu(Q_w), [Q_b;Q_b])


    final_w = affine_Q[2].weight  
    final_b = affine_Q[2].bias
    final_layer = Dense(weight_lu_2_l(final_w), final_b)
    
    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3),
        minmax_layer,
        neg_layer
    )

    b_low = Chain(
        filter_u,
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    model = Chain(
        Parallel(
            .*, 
            b1,
            b_low
            
        ),
        final_layer

    )


    return model
end



function create_parallel_affine_Q(x_dim, u_dim, hidden_sizes=32)
    # can take both x and xu as input
    # function filter_x(input)
    #     return input[1:x_dim, :]
    # end
    
    # function filter_u(input)
    #     return input[x_dim+1:end, :]
    # end
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # start_idx, end_idx, expand_dim
    b1_embedding_dim = 32
    W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
    b_expand_emb = zeros(b1_embedding_dim + u_dim)
    expand_emb = Dense(W_expand_emb, b_expand_emb)

    W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
    b_expand_u = zeros(b1_embedding_dim + u_dim)
    expand_u = Dense(W_expand_u, b_expand_u)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, 1)
    )
    # b2 = Chain(
    #     filter_u,
    #     # expand_u
    #     Dense(u_dim, 1)
    # )

    b2 = Chain(
        filter_u,
        # expand_u
        Dense(u_dim, hidden_sizes, relu),
        Dense(hidden_sizes, hidden_sizes, relu),
        Dense(hidden_sizes, 1)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))

    # Complete model
    model = Chain(
        Parallel(
            +, 
            b1,
            b2
        ),
        # final_layer  
    )
    return model
end



function create_mul_Q(x_dim, u_dim, hidden_sizes=32, output_emb_dim = 8)
    # can take both x and xu as input
    # function filter_x(input)
    #     return input[1:x_dim, :]
    # end
    
    # function filter_u(input)
    #     return input[x_dim+1:end, :]
    # end
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # start_idx, end_idx, expand_dim
    b1_embedding_dim = hidden_sizes
    W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
    b_expand_emb = zeros(b1_embedding_dim + u_dim)
    expand_emb = Dense(W_expand_emb, b_expand_emb)

    W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
    b_expand_u = zeros(b1_embedding_dim + u_dim)
    expand_u = Dense(W_expand_u, b_expand_u)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )

    b2 = Chain(
        filter_u,
        # expand_u
        Dense(u_dim, hidden_sizes, relu),
        Dense(hidden_sizes, hidden_sizes, relu),
        Dense(hidden_sizes, output_emb_dim)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))
    sum_layer = Dense(ones(1, output_emb_dim), zeros(1))
    # Complete model
    model = Chain(
        Parallel(
            .*, 
            b1,
            b2
        ),
        sum_layer
    )
    return model
end


function create_mul_affine_Q(x_dim, u_dim, hidden_sizes=32, output_emb_dim = 8)
    # can take both x and xu as input
    # function filter_x(input)
    #     return input[1:x_dim, :]
    # end
    
    # function filter_u(input)
    #     return input[x_dim+1:end, :]
    # end
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # start_idx, end_idx, expand_dim
    b1_embedding_dim = hidden_sizes
    W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
    b_expand_emb = zeros(b1_embedding_dim + u_dim)
    expand_emb = Dense(W_expand_emb, b_expand_emb)

    W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
    b_expand_u = zeros(b1_embedding_dim + u_dim)
    expand_u = Dense(W_expand_u, b_expand_u)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )

    b2 = Chain(
        filter_u,
        # expand_u
        Dense(u_dim, hidden_sizes),
        Dense(hidden_sizes, hidden_sizes),
        Dense(hidden_sizes, output_emb_dim)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))
    # sum_layer = Dense(ones(1, output_emb_dim), zeros(1))
    sum_layer = Dense(output_emb_dim, 1)
    # Complete model
    model = Chain(
        Parallel(
            .*, 
            b1,
            b2
        ),
        sum_layer
    )
    return model
end

function create_x_mul_xu_Q(x_dim, u_dim, hidden_sizes=32, output_emb_dim = 8)
    # can take both x and xu as input
    # function filter_x(input)
    #     return input[1:x_dim, :]
    # end
    
    # function filter_u(input)
    #     return input[x_dim+1:end, :]
    # end
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # start_idx, end_idx, expand_dim
    b1_embedding_dim = hidden_sizes
    W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
    b_expand_emb = zeros(b1_embedding_dim + u_dim)
    expand_emb = Dense(W_expand_emb, b_expand_emb)

    W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
    b_expand_u = zeros(b1_embedding_dim + u_dim)
    expand_u = Dense(W_expand_u, b_expand_u)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )

    b2 = Chain(
        Dense(u_dim+x_dim, hidden_sizes, relu),
        Dense(hidden_sizes, hidden_sizes, relu),
        Dense(hidden_sizes, output_emb_dim)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))
    sum_layer = Dense(ones(1, output_emb_dim), zeros(1))
    # Complete model
    model = Chain(
        Parallel(
            .*, 
            b1,
            b2
        ),
        sum_layer
    )
    return model
end

function create_x_mul_xu_Q_interval_old(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)
    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias
    minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))

    x_2_x_luul = Dense(replace_weight_xu_luul(x_dim, u_dim), replace_bias_xu_luul(u_high, u_low, x_dim, u_dim))

    h1_w = affine_Q[1][2][1].weight
    h1_b = affine_Q[1][2][1].bias
    x_luul_2_h1_luul = Dense(weight_luul(h1_w), bias_luul(h1_b), relu)

    h2_w = affine_Q[1][2][2].weight
    h2_b = affine_Q[1][2][2].bias   
    h1_luul_2_h2_luul = Dense(weight_luul(h2_w), bias_luul(h2_b), relu)


    Q_w = affine_Q[1][2][3].weight
    Q_b = affine_Q[1][2][3].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_lu = Dense(weight_lu_big(Q_w), [Q_b;Q_b])


    sum_layer = Dense(ones(1, 2*output_emb_dim), zeros(1))
    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3),
        minmax_layer,
        neg_layer
    )

    b_low = Chain(
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    model = Chain(
        Parallel(
            .*, 
            b1,
            b_low
            
        ),
        sum_layer

    )


    return model
end

function create_x_mul_xu_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias
    minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))

    x_2_x_luul = Dense(replace_weight_xu_lu(x_dim, u_dim), replace_bias_xu_lu(u_high, u_low, x_dim, u_dim))

    h1_w = affine_Q[1][2][1].weight
    h1_b = affine_Q[1][2][1].bias
    x_luul_2_h1_luul = Dense(weight_lu(h1_w), bias_lu(h1_b), relu)

    h2_w = affine_Q[1][2][2].weight
    h2_b = affine_Q[1][2][2].bias   
    h1_luul_2_h2_luul = Dense(weight_lu(h2_w), bias_lu(h2_b), relu)


    Q_w = affine_Q[1][2][3].weight
    Q_b = affine_Q[1][2][3].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_lu = Dense(weight_lu(Q_w), [Q_b;Q_b])


    sum_layer = Dense(ones(1, 2*output_emb_dim), zeros(1))
    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3),
        minmax_layer,
        neg_layer
    )

    b_low = Chain(
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    model = Chain(
        Parallel(
            .*, 
            b1,
            b_low
            
        ),
        sum_layer

    )


    return model
end

function create_x_add_xu_Q(x_dim, u_dim, hidden_sizes=32, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # start_idx, end_idx, expand_dim
    b1_embedding_dim = hidden_sizes
    W_expand_emb = create_expand_matrix(1, b1_embedding_dim, b1_embedding_dim + u_dim)
    b_expand_emb = zeros(b1_embedding_dim + u_dim)
    expand_emb = Dense(W_expand_emb, b_expand_emb)

    W_expand_u = create_expand_matrix(b1_embedding_dim + 1, b1_embedding_dim + u_dim, b1_embedding_dim + u_dim)
    b_expand_u = zeros(b1_embedding_dim + u_dim)
    expand_u = Dense(W_expand_u, b_expand_u)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )

    b2 = Chain(
        Dense(u_dim + x_dim, hidden_sizes),
        Dense(hidden_sizes, hidden_sizes),
        Dense(hidden_sizes, output_emb_dim)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))
    # sum_layer = Dense(ones(1, output_emb_dim), zeros(1))
    sum_layer = Dense(output_emb_dim, 1)
    # Complete model
    model = Chain(
        Parallel(
            +, 
            b1,
            b2
        ),
        sum_layer
    )
    return model
end

function create_x_add_xu_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    x_w1 = affine_Q[1][1][2].weight
    x_b1 = affine_Q[1][1][2].bias
    x_w2 = affine_Q[1][1][3].weight
    x_b2 = affine_Q[1][1][3].bias
    x_w3 = affine_Q[1][1][4].weight
    x_b3 = affine_Q[1][1][4].bias
    # minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    # neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))
    replicate_emb = Dense(weight_replicate_emb(output_emb_dim), bias_replicate_emb(zeros(2*output_emb_dim)))

    #TODO: Change to xu_2_xu_luul
    x_2_x_luul = Dense(replace_weight_xu_luul(x_dim, u_dim), replace_bias_xu_luul(u_high, u_low, x_dim, u_dim))

    h1_w = affine_Q[1][2][1].weight
    h1_b = affine_Q[1][2][1].bias
    x_luul_2_h1_luul = Dense(weight_luul(h1_w), bias_luul(h1_b))

    h2_w = affine_Q[1][2][2].weight
    h2_b = affine_Q[1][2][2].bias   
    h1_luul_2_h2_luul = Dense(weight_luul(h2_w), bias_luul(h2_b))


    Q_w = affine_Q[1][2][3].weight
    Q_b = affine_Q[1][2][3].bias
    # println("shape of Q_w", size(Q_w))
    h2_luul_2_Q_lu = Dense(weight_lu(Q_w), [Q_b;Q_b])


    final_w = affine_Q[2].weight  
    final_b = affine_Q[2].bias
    final_layer = Dense(weight_lu_2_l(final_w), final_b)
    
    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_w1, x_b1, relu),
        Dense(x_w2, x_b2, relu),
        Dense(x_w3, x_b3),
        replicate_emb
    )

    b_low = Chain(
        x_2_x_luul,
        x_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    model = Chain(
        Parallel(
            .*, 
            b1,
            b_low
            
        ),
        final_layer

    )


    return model
end

function create_x_add_x_mul_u_Q(x_dim, u_dim, hidden_sizes=32, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    # Branch 1
    b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )

    b2 = Chain(
        Dense(u_dim, hidden_sizes),
        Dense(hidden_sizes, hidden_sizes),
        Dense(hidden_sizes, output_emb_dim)
    )

    b3 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )
    # final_layer = Chain(Dense(32 + u_dim, 1))
    # sum_layer = Dense(ones(1, output_emb_dim), zeros(1))
    sum_layer = Dense(output_emb_dim, 1)
    # Complete model
    model = Chain(
        Parallel(
            +,
            Parallel(
                .*, 
                b1,
                b2
            ),
            b3
        ),
        sum_layer
        
    )
    return model
end

function create_x_add_x_mul_u_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high, output_emb_dim = 8)
    W_x = create_filter_matrix(1, x_dim, x_dim + u_dim)
    b_x = zeros(x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(x_dim + 1, x_dim + u_dim, x_dim + u_dim)
    u_b = zeros(u_dim)
    filter_u = Dense(W_u, u_b)

    b1_l2_w = affine_Q[1][1][1][1][2].weight
    b1_l2_b = affine_Q[1][1][1][1][2].bias
    b1_l3_w = affine_Q[1][1][1][1][3].weight
    b1_l3_b = affine_Q[1][1][1][1][3].bias
    b1_l4_w = affine_Q[1][1][1][1][4].weight
    b1_l4_b = affine_Q[1][1][1][1][4].bias

    b1 = affine_Q[1][1][1][1]

    b2_l2_w = affine_Q[1][1][1][2][2].weight
    b2_l2_b = affine_Q[1][1][1][2][2].bias
    b2_l3_w = affine_Q[1][1][1][2][3].weight
    b2_l3_b = affine_Q[1][1][1][2][3].bias
    b2_l4_w = affine_Q[1][1][1][2][4].weight
    b2_l4_b = affine_Q[1][1][1][2][4].bias
    
    b2 = affine_Q[1][1][1][2]

    b3_l2_w = affine_Q[1][1][2][2].weight
    b3_l2_b = affine_Q[1][1][2][2].bias
    b3_l3_w = affine_Q[1][1][2][3].weight
    b3_l3_b = affine_Q[1][1][2][3].bias
    b3_l4_w = affine_Q[1][1][2][4].weight
    b3_l4_b = affine_Q[1][1][2][4].bias

    b3 = affine_Q[1][1][2]

    final_w = affine_Q[1][2].weight
    final_b = affine_Q[1][2].bias

    
    
    #structure: (b1 .* b2) .+ b3
    #TODO: create layers
    u_2_u_luul = Dense(u_dim, 4*u_dim; bias = [u_low; u_high; u_high; u_low])
    u_2_u_luul.weight .= 0.0
    
    minmax_layer = Dense(weight_pos_neg(output_emb_dim), zeros(2*output_emb_dim), relu)
    neg_layer = Dense(weight_neg(output_emb_dim), zeros(2*output_emb_dim))
    replicate_emb = Dense(weight_replicate_emb(output_emb_dim), bias_replicate_emb(zeros(2*output_emb_dim)))


    u_luul_2_h1_luul = Dense(weight_luul(b3_l2_w), bias_luul(b3_l2_b), relu)
   
    h1_luul_2_h2_luul = Dense(weight_luul(b3_l3_w), bias_luul(b3_l3_b), relu)

    h2_luul_2_Q_lu = Dense(weight_lu(b3_l4_w), [b3_l4_b;b3_l4_b])

    final_layer = Dense(weight_lu_2_l(final_w), final_b)
    
    base_b1 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )
    Flux.copy!(params(base_b1), params(b1))

    base_b3 = Chain(
        filter_x,
        Dense(x_dim, 32, relu),
        Dense(32, 32, relu),
        Dense(32, output_emb_dim)
    )
    Flux.copy!(params(base_b3), params(b3))
    
    base_b2 = Chain(
        filter_u,
        u_2_u_luul,
        u_luul_2_h1_luul,
        h1_luul_2_h2_luul,
        h2_luul_2_Q_lu
    )
    
    model = Chain(
        Parallel(
            +,
            Parallel(
                .*,
                Chain(
                    base_b1,
                    minmax_layer,
                    neg_layer
                ),
                base_b2
            ),

            Chain(
                base_b3,
                replicate_emb
            )
        ),
        final_layer
    )
    return model
end

function create_baseline_affine_Q(x_dim, u_dim, hidden_sizes=[32, 32])
    model = create_mlp(x_dim + u_dim, 1, hidden_sizes)
    return model
end

function create_baseline_affine_Q_interval(affine_Q, x_dim, u_dim, u_low, u_high)
    x_2_x_luul = Dense(replace_weight_xu_lu(x_dim, u_dim), replace_bias_xu_lu(u_high, u_low, x_dim, u_dim))

    h1_w = affine_Q[1].weight
    h1_b = affine_Q[1].bias
    x_lu_2_h1_lu = Dense(weight_lu(h1_w), bias_lu(h1_b))

    h2_w = affine_Q[2].weight
    h2_b = affine_Q[2].bias   
    h1_lu_2_h2_lu = Dense(weight_lu(h2_w), bias_lu(h2_b))


    h3_w = affine_Q[3].weight
    h3_b = affine_Q[3].bias
    h2_lu_2_h3_l = Dense(weight_l_small(h3_w), h3_b)
    

    model = Chain(
        x_2_x_luul,
        x_lu_2_h1_lu,
        h1_lu_2_h2_lu,
        h2_lu_2_h3_l
    )
    return model
end

function create_baseline_affine_Q_argmax(affine_Q, x_dim, u_dim, u_low, u_high)
    x_2_x_luul = Dense(replace_weight_xu_lu(x_dim, u_dim), replace_bias_xu_lu(u_high, u_low, x_dim, u_dim))

    h1_w = affine_Q[1].weight
    h1_b = affine_Q[1].bias
    x_lu_2_h1_lu = Dense(weight_lu(h1_w), bias_lu(h1_b))

    h2_w = affine_Q[2].weight
    h2_b = affine_Q[2].bias   
    h1_lu_2_h2_lu = Dense(weight_lu(h2_w), bias_lu(h2_b))


    h3_w = affine_Q[3].weight
    h3_b = affine_Q[3].bias
    h2_lu_2_h3_u = Dense(weight_u_small(h3_w), h3_b)
    

    model = Chain(
        x_2_x_luul,
        x_lu_2_h1_lu,
        h1_lu_2_h2_lu,
        h2_lu_2_h3_u
    )
    return model
end
    

function create_Q_constraint_model(Q_model, h_model, task)
    # function filter_x(input)
    #     return input[1:task.x_dim, :]
    # end

    W_x = create_filter_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    b_x = zeros(task.x_dim)
    filter_x = Dense(W_x, b_x)
    return Chain(Parallel(+,
        Chain(Q_model,  Dense(Float32[1; 0;;])),
        Chain(filter_x, h_model, Dense(Float32[0; 1;;]))
    ))
end

function create_argmin_Q_constraint_model(Q_model, h_model, task)
    # function filter_x(input)
    #     return input[1:task.x_dim, :]
    # end

    W_x = create_filter_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    b_x = zeros(task.x_dim)
    filter_x = Dense(W_x, b_x)
    W_u = create_filter_matrix(1, task.u_dim, task.u_dim)
    affine_Q_interval = create_x_mul_xu_Q_interval(Q_model, task.x_dim, task.u_dim, task.u_low, task.u_high)

    return Chain(Parallel(+,
        Chain(affine_Q_interval,  Dense(Float32[1; 0;;])),
        Chain(filter_x, h_model, Dense(Float32[0; 1;;]))
    ))
end

function create_Q_Q_prime(affine_Q, f_pi_model, f_model, task)
    # creating Q_prime
    # affine_Q_interval = create_parallel_affine_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_mul_affine_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_mul_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_x_mul_xu_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_x_add_xu_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    affine_Q_interval = create_baseline_affine_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    W_x = create_filter_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    b_x = zeros(task.x_dim)
    filter_x = Dense(W_x, b_x)
    
    # expand_layer = create_expand_xu_layer(task.x_dim, task.u_dim)
    expand_W = create_expand_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    expand_b = zeros(task.x_dim + task.u_dim)
    expand_layer = Dense(expand_W, expand_b)
    # println("PASS 2")
    return Chain(Parallel(+,
        Chain(affine_Q, Dense(Float32[1; 0;;])),
        # Chain(filter_x, f_pi_model, expand_layer, affine_Q_interval, Dense(Float32[0; 1;;])),
        Chain(f_model, expand_layer, affine_Q_interval, Dense(Float32[0; 1;;])),
    )), affine_Q_interval
end


function create_Q_Q_max_prime(affine_Q, f_pi_model, f_model, task)
    # creating Q_Q_max_prime
    # affine_Q_interval = create_parallel_affine_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_mul_affine_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_mul_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_x_mul_xu_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    # affine_Q_interval = create_x_add_xu_Q_interval(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    Q_max = create_baseline_affine_Q_argmax(affine_Q, task.x_dim, task.u_dim, task.u_low, task.u_high)
    W_x = create_filter_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    b_x = zeros(task.x_dim)
    filter_x = Dense(W_x, b_x)
    
    # expand_layer = create_expand_xu_layer(task.x_dim, task.u_dim)
    expand_W = create_expand_matrix(1, task.x_dim, task.x_dim + task.u_dim)
    expand_b = zeros(task.x_dim + task.u_dim)
    expand_layer = Dense(expand_W, expand_b)
    # println("PASS 2")
    return Chain(Parallel(+,
        Chain(affine_Q, Dense(Float32[1; 0;;])),
        # Chain(filter_x, f_pi_model, expand_layer, affine_Q_interval, Dense(Float32[0; 1;;])),
        Chain(f_model, expand_layer, Q_max, Dense(Float32[0; 1;;])),
    )), Q_max
end