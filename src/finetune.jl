...
function finetune_Q(
    ...
    for i in ProgressBar(1:max_iter)
        ...
        #TODO: separate the constraint and invariance counterexample search
        # if ((length(con_buffer.stored) + length(inv_buffer.stored)) < search_stop)


        #     #TODO: constraint counterexample search
        #     x = uniform(x_low, x_high, round(Int64, search_size / bnd_ratio))
        #     v = Q_model(x)[1, :]
        #     x_bnd = x[:, (v .> -bnd_eps) .& (v .<= tol)]
        #     con_bnd_ratio = bnd_ratio_avg * con_bnd_ratio + (1 - bnd_ratio_avg) * size(x_bnd, 2) / size(x, 2)
        #     con_bnd_ratio = clamp(con_bnd_ratio, min_bnd_ratio, max_bnd_ratio)

        #     if search_method == "BGB"
        #         con_x_pgd = boundary_guided_search_Q(task, x_bnd, x_low, x_high, h_model, Q_model, affine_Q_interval, f_pi_model;
        #         f_model = f_model, pgd_step=pgd_step, pgd_eps=pgd_eps, backtrack_step=backtrack_step,
        #             length_discount=length_discount, bound_guide=true, direct_discount=direct_discount,
        #             tol=tol, mode="con")
        #     end
        #     con, _ = filter_counterexample_Q(task, con_x_pgd, h_model, Q_model, affine_Q_interval, f_pi_model; f_model = f_model, tol=tol)




        #     #TODO: invariance counterexample search
        #     x = uniform(x_low, x_high, round(Int64, search_size / bnd_ratio))
        #     v = Q_model(x)[1, :]
        #     x_bnd = x[:, (v .> -bnd_eps) .& (v .<= tol)]
        #     inv_bnd_ratio = bnd_ratio_avg * inv_bnd_ratio + (1 - bnd_ratio_avg) * size(x_bnd, 2) / size(x, 2)
        #     inv_bnd_ratio = clamp(inv_bnd_ratio, min_bnd_ratio, max_bnd_ratio)

        #     if search_method == "BGB"
        #         inv_x_pgd = boundary_guided_search_Q(task, x_bnd, x_low, x_high, h_model, Q_model, affine_Q_interval, f_pi_model;
        #         f_model = f_model, pgd_step=pgd_step, pgd_eps=pgd_eps, backtrack_step=backtrack_step,
        #             length_discount=length_discount, bound_guide=true, direct_discount=direct_discount,
        #             tol=tol, mode="inv")
        #     end
        #     _, inv = filter_counterexample_Q(task, inv_x_pgd, h_model, Q_model, affine_Q_interval, f_pi_model; f_model = f_model, tol=tol)
            
        #     push!(con_buffer, con_x_pgd[:, con])
        #     push!(inv_buffer, inv_x_pgd[:, inv])
        #     push!(buffer, con_x_pgd[:, con])
        #     push!(buffer, inv_x_pgd[:, inv])
        ...
        if length(buffer.stored) > 0
            ...
            function con_loss_fn(Q_model)
                ...
                if n_arg_con > 0
                    arg_con_loss = -affine_Q_interval(x_arg_con)
                else
                    arg_con_loss = 0
                end
                if n_inv > 0
        ...
        #             reg_loss = 0
        #         end
        #         loss = (con_loss) / max(n_con, 1) + reg_coef * reg_loss
        #         return loss
        #     end
        ...
        #     Optimisers.update!(opt_state_all, Q_model, grad[1])
        #     Q_Q_prime_model, affine_Q_interval = create_Q_Q_prime(Q_model, f_pi_model, f_model, task)
        #     # Q_h_model = create_argmin_Q_constraint_model(Q_model, h_model, task)
        #     Q_h_model = create_Q_constraint_model(Q_model, h_model, task)
        ...
        #         if (isnothing(early_stop)) | (inv_update < early_stop)
        #             Optimisers.update!(opt_state_all, Q_model, grad[1])
        #         else
        #             Optimisers.update!(opt_state_u, Q_model, grad[1])
        #         end
